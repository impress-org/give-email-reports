...
class Give_Weekly_Email_Notification extends Give_Email_Notification {
	...
	public function setup_email_data( $form_id = null ) {
		$heading = empty( $form_id )
			? sprintf( __( 'Weekly Donation Report <br> %s', 'give-email-reports' ), get_bloginfo( 'name' ) )
			: sprintf( __( 'Weekly Donation Report for <br> "%s"', 'give-email-reports' ), get_the_title( $form_id ) );
